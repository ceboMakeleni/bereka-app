...
export default function DashboardShell({ children, isAdmin, username }: DashboardShellProps) {
    ...
    const NavLinks = ({ mobile = false }: { mobile?: boolean }) => (
        <>
            ...
            {isAdmin && (
                ...
                >
                    <ShieldCheck className={`h-4 w-4 ${isActive('/dashboard/admin') ? 'text-primary' : ''}`} />
    ...
    return (
        <div className="flex min-h-screen w-full flex-col bg-background md:flex-row font-sans">
            {/* Desktop Sidebar */}
            <aside className="fixed inset-y-0 left-0 z-10 hidden w-16 flex-col border-r bg-card/50 backdrop-blur-xl sm:flex md:w-64 transition-all duration-300">
                ...
                {username && (
                    <div className="px-5 py-4 border-b border-border/50 flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-primary to-accent flex items-center justify-center text-primary-foreground font-semibold shadow-sm hidden md:flex">
            ...
            {/* Mobile Drawer */}
            <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-card border-r shadow-2xl transform transition-transform duration-300 cubic-bezier(0.4, 0, 0.2, 1) sm:hidden flex flex-col ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
                }`}>
            ...
            <div className="flex flex-col flex-1 sm:gap-4 sm:py-4 sm:pl-16 md:pl-64 min-w-0">
                <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b bg-background/80 backdrop-blur-xl px-4 sm:static sm:h-auto sm:border-0 sm:bg-transparent sm:px-8 transition-all duration-200">
                    ...
                    <div className="font-bold text-lg sm:hidden flex items-center gap-2">
                ...
                <main className="flex-1 items-start p-4 sm:px-8 sm:py-2 max-w-7xl mx-auto w-full">
